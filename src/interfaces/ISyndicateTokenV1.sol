...
interface ISyndicateTokenV1 {
    ...
    /// @dev Dissolved Syndicates' tokens will continue to 'function' in that they can continue to be transferred, but they will no longer be legible to the registry contract and a new Syndicate Token may be launched by the same Urbit ID to occupy that slot in the SyndicateRegistry contract.
